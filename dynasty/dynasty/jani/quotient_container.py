...
from dynasty.model_handling.mdp_handling import ModelHandling, always_true
...
def is_inside_function(threshold):
    return lambda lower, upper: lower < float(threshold) < upper
...
class JaniQuotientContainer:
    ...
    def consider_subset(self, subset, indexed_suboptions):
        ...
        single_family_member = max([len(options) for options in subset.values()]) == 1
        if single_family_member:
            logger.debug("Submodel reflects a single family member: {}".format(
                ",".join(["{}={}".format(k, v[0]) for k, v in subset.items()])))
        start_time = time.time()

        sub_colors = self._edge_coloring.subcolors(indexed_suboptions)
    ...
    def _compute_differences(self, scheduler_selection_0, scheduler_selection_1):
        ...
        for h, counts_0 in scheduler_selection_0.items():
            if h not in scheduler_selection_1:
                diffs[h] = sum(counts_0.values()) / 8 if len(counts_0) > 1 else 0.0
        ...
        for h in self._ordered_holes:
            if h in self._inconsistencies[0] and h in self._inconsistencies[1]:
                if True:  # self._inconsistencies[0][h][1] != self._inconsistencies[1][h][1]:
                    if self._inconsistencies[0][h][0] + self._inconsistencies[1][h][0] + self._inconsistencies[0][h][2]\
                            + self._inconsistencies[1][h][2] > propose_split_score:
    ...
    def scheduler_color_analysis(self, index=0):
        ...
        assert self._latest_results[index].scheduler is not None
        assert self._latest_results[index].alt_scheduler is not None

        self._hole_option_maps = []
        self._inconsistencies = []

        for scheduler in [self._latest_results[index].scheduler, self._latest_results[index].alt_scheduler]:
            ...
            # hole_option_map = self._selected_hole_counts(
            #       color_map, latest_result.lower_bound_result, latest_result.upper_bound_result
            #   )

            hole_option_map = self._selected_hole_counts_incremental(
                state_map, action_map,
                self._latest_results[index].lower_bound_result,
                self._latest_results[index].upper_bound_result
            )
    ...
    def _model_color_analysis_acts(self, mdp):
        ...
        for action_index in range(mdp.nr_choices):
            aut_edge_indices = mdp.choice_origins.get_edge_index_set(action_index)
            act_to_color[action_index] = set([self._color_for_choice_origin(e) for e in aut_edge_indices])
    ...
    def _selected_hole_counts_incremental(self, state_map, action_map, mc_lower_result, mc_upper_result):
        ...
        difference = mc_lower_result.at(self._mdp_handling.full_mdp.initial_states[0]) - \
            mc_upper_result.at(self._mdp_handling.full_mdp.initial_states[0])
        if difference < 0.001:
    ...
    def decided(self, formulae, thresholds):
        ...
        for latest_result, formula, threshold in (zip(self._latest_results, formulae, thresholds)):
            ...
            if float(threshold) > latest_result.absolute_max:
                logger.debug(f"Absolute maximum {latest_result.absolute_max} is below threshold {threshold}")
                decisions.append(ThresholdSynthesisResult.BELOW)
            elif float(threshold) <= latest_result.absolute_min:
                logger.debug(f"Absolute minimum {latest_result.absolute_min} is above threshold {threshold}")
                decisions.append(ThresholdSynthesisResult.ABOVE)
            else:
                logger.debug("Threshold is between")
                decisions.append(ThresholdSynthesisResult.UNDECIDED)

        return decisions

    def upper_bound(self, index=0):
        return self._latest_results[index].absolute_max

    def lower_bound(self, index=0):
        return self._latest_results[index].absolute_min
    ...
    def _lower_bound_inconsistencies(self, index=0):
        return self._inconsistencies[1] if self._latest_results[index].maximising else self._inconsistencies[0]
    ...
    def _upper_bound_inconsistencies(self, index=0):
        return self._inconsistencies[0] if self._latest_results[index].maximising else self._inconsistencies[1]
    ...
    def analyse(self, threshold=None, index=0, engine=Engine.Sparse):
        ...
        if engine == engine.Dd:
            self._latest_results[index] = self._mdp_handling.mc_model_symbolic(index)
        elif engine == engine.Hybrid:
            self._latest_results[index] = self._mdp_handling.mc_model_hybrid(index)
        else:
            assert engine == Engine.Sparse
            self._latest_results.append(self._mdp_handling.mc_model(
                index, compute_action_values=False, check_dir_2=always_true  # TODO: is_inside_function?
            ))
        end_time = time.time()
        self._mc_time += end_time - start_time
        return self._latest_results[index]

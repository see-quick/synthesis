...
class ExplicitMCResult:
    def __init__(
            self, result, alt_result, maximising=True, prime_result_action_values=None,
            second_result_action_values=None, absolute_min=None, absolute_max=None):
        self.result = result
        self.alt_result = alt_result
        self.maximising = maximising
        self.absolute_min = absolute_min
        self.absolute_max = absolute_max
        self.prime_result_action_values = prime_result_action_values
        self.second_result_action_values = second_result_action_values
...
class ModelHandling:
    ...
    def display_model(self):
        ...
        if pgv is not None:
            ...
            logger.debug("Missing <pygraphviz> package. Please install it and then run again.")
    ...
    def restrict(self, edge_indices, edge_0_indices):
        ...
        submodel_construction = stormpy.construct_submodel(self._model, all_states, selected_actions,
                                                           keep_unreachable_states, subsystem_options)
    ...
    def mc_model(self, index=0, compute_action_values=False, check_dir_2=always_true):
        ...
        if is_dtmc:
            ...
            return ExplicitMCResult(
                prime_result, prime_result, maximise, None, None, absolute_min=absolute_min, absolute_max=absolute_max
            )
        ...
        if check_dir_2(absolute_min, absolute_max):
            ...
            if maximise:
                absolute_min = min([second_result.at(x) for x in self._submodel.initial_states])
            else:
                absolute_max = max([second_result.at(x) for x in self._submodel.initial_states])
        ...
        if compute_action_values:
            prime_result_action_values = \
                self._submodel.transition_matrix.multiply_with_vector(prime_result.get_values())
            second_result_action_values = \
                self._submodel.transition_matrix.multiply_with_vector(second_result.get_values())

        return ExplicitMCResult(
            prime_result, second_result, maximise, prime_result_action_values, second_result_action_values,
